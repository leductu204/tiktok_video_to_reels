    ...
    axios
      ...
      .then(async ({ request }) => {
        ...
        if (content.image_post_info) {
          ...
          // Video Result
          // Create video array with quality information
          const videoWithQualities = content.video.play_addr.url_list.map((url, index) => {
            // Extract quality information or provide default values
            let quality;
            if (index === 0) {
